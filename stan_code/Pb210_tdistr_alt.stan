// version w/ two log densities summed
functions {
...
transformed data {
  ...
  matrix[N,N] Sigma_scal; //diagonal matrix for variances for sigma_tot
  matrix[L,L] SigmaL_scal; //variances for sigma_tot scaled by b/a
  ...
  Sigma_scal = Sigma *b/a;
  SigmaL_scal = SigmaL *b/a;
}
...
model {
  ...
  target += multi_student_t_lpdf(y | df, mu, Sigma_scal)+multi_student_t_lpdf(yL | df, muL, SigmaL_scal);
