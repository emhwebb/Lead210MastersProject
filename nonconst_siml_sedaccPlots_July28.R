...
nonconst_supp_norm_gam_initJuly28_accurate <- 1/nonconst_supp_norm_gam_initJuly28_accurate[,2:ncol(nonconst_supp_norm_gam_initJuly28_accurate)]
nonconst_supp_t_gam_initJuly28_accurate <- 1/nonconst_supp_t_gam_initJuly28_accurate[,2:ncol(nonconst_supp_t_gam_initJuly28_accurate)]
nonconst_supp_nonconst_norm_gam_initJuly28_accurate <- 1/nonconst_supp_nonconst_norm_gam_initJuly28_accurate[,2:ncol(nonconst_supp_nonconst_norm_gam_initJuly28_accurate)]
nonconst_supp_nonconst_t_gam_initJuly28_accurate <- 1/nonconst_supp_nonconst_t_gam_initJuly28_accurate[,2:ncol(nonconst_supp_nonconst_t_gam_initJuly28_accurate)]
...
siml_mar_gam_df <- as.data.frame(cbind(Depth, True_SedAcc, 
                                       siml_gam_normal_mean, siml_gam_tdistr_mean)) %>% 
  select(Depth, True_SedAcc, Normal, tdistr) %>%
  pivot_longer(cols = c("True_SedAcc", "Normal", "tdistr"), 
               names_to = "Method", values_to = "Age") 

siml_gam_plot_df <- left_join(siml_mar_gam_df, siml_gam_bci, by = c("Depth", "Method"))

siml_mar_nonconst_df <- as.data.frame(cbind(Depth, True_SedAcc, 
...
p1 <- siml_nonconst_plot_df %>%
  ...
  scale_y_continuous(name= bquote("Median Acc Rate " (~cm~yr^-1)), breaks = seq(0, 250, 1))+
  theme_bw()+
...
p2 <- siml_gam_plot_df %>%
  ...
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), size = 1, alpha = 0.4)+
  scale_x_continuous(name="Depth (cm)", breaks = seq(0, 30, 5)) +
  scale_y_continuous(name= bquote("Median Acc rate " (~cm~yr^-1)), breaks = seq(0, 20, 1))+
  theme_bw()+
...
siml_mar_const_gam_df <- as.data.frame(cbind(Depth, True_SedAcc, 
...
siml_mar_const_nonconst_df <- as.data.frame(cbind(Depth, True_SedAcc, 
...
p4 <- siml_const_gam_plot_df %>%
  ...
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), size = 1, alpha = 0.4)+
  scale_x_continuous(name="Depth (cm)", breaks = seq(0, 30, 5)) +
  scale_y_continuous(name= bquote("Median Acc Rate " (~cm~yr^-1)), breaks = seq(0, 30, 1.5))+
  theme_bw()+
...
p5 <- siml_const_nonconst_plot_df %>%
  ...
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), size = 1, alpha = 0.4)+
  scale_x_continuous(name="Depth (cm)", breaks = seq(0, 30, 5)) +
  scale_y_continuous(name= bquote("Median Acc Rate " (~cm~yr^-1)), breaks = seq(0, 20, 1.5))+
  theme_bw()+
