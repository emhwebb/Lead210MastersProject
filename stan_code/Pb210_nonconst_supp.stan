...
parameters {
  ...
  vector<lower = 0>[L] Ps; //supported activity
  real<lower = 0> mN; //m_N^th accumulation rate
  }
  
transformed parameters {
   vector<lower = 0>[N] A; //supported activity. note: should this be N or L for indexing?
...
model {
  ...
  for(n in 1:L){ 
    Ps[n] ~  gamma(alpha_P, beta_P); //prior for gamma parameter
  }
